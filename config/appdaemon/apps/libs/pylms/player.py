...
class Player(object):
    ...
    def playlist_resume(self, playlist):
        """Resume une playliste"""
        item = self.__quote(playlist)
        return self.request("playlist resume %s noplay:0" % (item))

    def get_playlist_shuffle_state(self):                            
        """Etat shuffle dune playlist"""                             
        return self.request("playlist shuffle ?")                    

    def get_playlist_index(self):                                    
        """Etat shuffle dune playlist"""                             
        return self.request("playlist index ?")                      

    def set_playlist_shuffle_state(self,state):                      
        """Etat shuffle dune playlist"""                             
        return self.request("playlist shuffle %i" % (int(state)))    

    def playlist_save(self,item):
        """Sauver Playlist"""
        item = self.__quote(item)
        self.request("playlist save %s silent:1" % (item))

    def playlist_play_playlist(self, item):
        """Play Item Immediately"""
        item = self.__quote(item)
        self.request("playlistcontrol cmd:load playlist_name:%s" % (item))
